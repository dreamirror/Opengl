...
int main()
{
	...
	glGenVertexArrays(1, &cubeVAO); //申请VAO
	glGenBuffers(1, &cubeVBO);//申请VBO
	glBindVertexArray(cubeVAO); //绑定VAO
	glBindBuffer(GL_ARRAY_BUFFER, cubeVBO); //绑定VBO
	glBufferData(GL_ARRAY_BUFFER, sizeof(cubeVertices), &cubeVertices, GL_STATIC_DRAW); //上传数据到VBO
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)0); //设置内存格式
	...
	unsigned int vegetationTextture = loadTexture("res/textures/window.png");
	shader.use();
	...
	//关联 Uniform块 和uniform缓冲到绑定点

	//获得shader的uniform内存块的索引
	unsigned int uniformBlockIndexCube = glGetUniformBlockIndex(shader.ID,"Matrices");
	unsigned int uniformBlockIndexSkybox = glGetUniformBlockIndex(skyboxShader.ID,"Matrices");

	//将shader的uniform内存块绑定到绑定点
	glUniformBlockBinding(shader.ID,uniformBlockIndexCube,0); 
	...
	glBufferData(GL_UNIFORM_BUFFER,2*sizeof(glm::mat4),NULL,GL_STATIC_DRAW);//申请UBO的内存
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		processInput(window);
		glBindFramebuffer(GL_FRAMEBUFFER,framebuffer); //启用自己的帧缓冲 而不是自己的默认缓冲
		...
		//启用默认的帧缓冲将刚刚渲染到自己的帧缓冲的东西渲染到默认的帧缓冲
		glBindFramebuffer(GL_FRAMEBUFFER,0);
		glDisable(GL_DEPTH_TEST);//因为现在只是渲染一张图片所以就不同深度测试了
