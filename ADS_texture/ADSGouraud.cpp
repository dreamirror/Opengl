...
void SetupRC(void)
	...
	ADSLightShader = shaderManager.LoadShaderPairWithAttributes("ADSGouraud.vp", "ADSGouraud.fp", 2, GLT_ATTRIBUTE_VERTEX, "vVertex", //2表示 每个定点有两属性 法线和颜色
...
void RenderScene(void)
    ...
    modelViewMatrix.PushMatrix(viewFrame);
		...
		glUniformMatrix4fv(locMVP, 1, GL_FALSE, transformPipeline.GetModelViewProjectionMatrix()); //后去模型视图投影矩阵
		glUniformMatrix4fv(locMV, 1, GL_FALSE, transformPipeline.GetModelViewMatrix()); //绑定模型矩阵
		glUniformMatrix3fv(locNM, 1, GL_FALSE, transformPipeline.GetNormalMatrix()); //定点的法线矩阵
    sphereBatch.Draw(); //绘制
